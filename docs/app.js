...
function variance(arr){
  ...
  const result = arr.map(x=> (x-mu)**2 ).reduce((a,b)=>a+b,0)/arr.length;
  return Math.round(result);
}

function sdev(arr){
  return Math.round(Math.sqrt(variance(arr)));
}
...
function compute(){
  ...
  out.innerHTML = `
    ...
    <div class="result-item">
      ...
      <span class="result-value">${average(vals)}</span>
    </div>
    <div class="result-item">
      <span class="result-label">Median:</span>
      <span class="result-value">${median(vals)}</span>
    </div>
    <div class="result-item">
      <span class="result-label">Variance:</span>
      <span class="result-value">${variance(vals)}</span>
    </div>
    <div class="result-item">
      <span class="result-label">Std Dev:</span>
      <span class="result-value">${sdev(vals)}</span>
    </div>
  `;
}